...
%% checking new nodes.
 -spec init(list(pid())) -> {ok, list(pid())}.
init(Nodes) ->
    ?DBG("starting gen_server with nodes: " ++ ?FROM_PIDS_TO_STRING(Nodes)),
...
handle_call(nodes_request, From, Nodes) ->
    ?DBG("requested nodes (my nodes: " ++ ?FROM_PIDS_TO_STRING(Nodes) ++ ")"),
...
handle_call(stop, _From, Nodes) ->
    ?DBG("stopping (my nodes was: " ++ ?FROM_PIDS_TO_STRING(Nodes) ++ ")"),
...
handle_cast({nodes_response, New_nodes}, Nodes) ->
    ?DBG("received new neightbors: " ++ ?FROM_PIDS_TO_STRING(New_nodes) ++ " (my new nodes: " ++
        ?FROM_PIDS_TO_STRING(merge_nodes(New_nodes, Nodes)) ++ ")"),
    {noreply, merge_nodes(New_nodes, Nodes)};


% If a node send an unknown message via cast, then don't do anything.
handle_cast(_Request, State) ->
    ?DBG("received unknown message (my nodes: " ++ ?FROM_PIDS_TO_STRING(State) ++ ")"),
    {noreply, State}.
...
handle_info(time_to_check_nodes, Nodes) ->
    ?DBG("timeout, it's searching nodes time! (my nodes: " ++ ?FROM_PIDS_TO_STRING(Nodes) ++ ")"),
...
send_nodes_request(Nodes) ->
    ...
    Is_alive = is_process_alive(Send_to),
    if 
        Is_alive ->
            Nodes;
        true ->
...
-spec merge_nodes(New_neighbors:: [pid()], Old_neighbord:: [pid()]) -> [pid()].
