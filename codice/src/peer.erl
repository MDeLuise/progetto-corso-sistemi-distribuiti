...
-export([start/1, start/2, start/4, stop/1, add_nodes/2, send/3, send/4]).
-export([add_trans/6]). % here just for test

-include("debug.hrl").
-include("peer_types.hrl").

-record(transaction, {id               :: trans_id(),
                      state = pending  :: trans_state(),
                      seq_number       :: trans_seq(),
                      guard            :: guard(),
                      command          :: command()
}).

-record(tran_aux_info, {asked_this = []                :: list(pid()),
                        max_prop_seq                   :: trans_seq(),
                        waiting_response = dict:new()  :: dict:dict(pid(), atom()),
                        current_seq                    :: trans_seq()
}).

-record(state, {nodes = []                   :: list(pid()),
                transactions = orddict:new() :: orddict:orddict(trans_seq(), #transaction{}),
                vars                         :: #variables{},
                sending_tran = nothing       :: {just, #transaction{}} | nothing,
                monitoring = []              :: list({pid(), #transaction{}}),
                monitored  = []              :: list(pid()),
                candidates = dict:new()      :: dict:dict(pid(), atom()),
                tran_infos = dict:new()      :: dict:dict(trans_id(), #tran_aux_info{}),
...
%% returned.
-spec start(Vars:: #variables{}, Nodes:: list(pid())) -> pid().
start(Vars, Nodes) ->
    process_flag(trap_exit, true), % here so we can kill the spawned peer froms shell without
                                   % being killed even our self
    {ok, Pid} = gen_server:start(?MODULE, {#state{vars = Vars}, Nodes}, []),
    Pid.


-spec start(Vars:: #variables{}, Nodes:: list(pid()), Init:: [({guard(), guard(), command()} |
    { guard(), command()})], Loop:: [{guard(), guard(), command()} | {guard(), command()}]) -> pid().
...
handle_call(stop, _Reason, State) ->
    ?DBG("stopping (my nodes was: " ++ ?FROM_PIDS_TO_STRING(State#state.nodes) ++ ")"),
    {stop, normal, ok, State}.


handle_cast({nodes_request, Pid}, State) ->
    ?DBG("received nodes_request (my nodes: " ++ ?FROM_PIDS_TO_STRING(State#state.nodes) ++ ")"),
...
handle_cast({nodes_response, New_neighbors}, State) ->
    ?DBG("received nodes_response (my new nodes: " ++
        ?FROM_PIDS_TO_STRING(merge_neighbors(New_neighbors, State#state.nodes)) ++ ")"),
...
handle_cast({link_require, Pid}, State) ->
    ...
    New_neighbors =
        if
            ...
            true ->
                link(Pid), %maybe not needed because Pid has already link me
                State#state.nodes ++ [Pid]
        end,
    ?DBG("received link_require from " ++ ?FROM_PIDS_TO_STRING([Pid]) ++ " (my new nodes: " ++
        ?FROM_PIDS_TO_STRING(New_neighbors) ++ ")"),
    {noreply, State#state{nodes = New_neighbors}};


handle_cast({gossip_me, Last_trans, Pid}, State) ->
    ?DBG("received gossip_me from " ++ ?FROM_PIDS_TO_STRING([Pid]) ++
    ...
    if
        length(Trans_to_send) > 0 ->
            ?DBG("sent gossip_you to " ++ ?FROM_PIDS_TO_STRING([Pid])),
...
handle_cast({let_me_gossip, Last_pending, Pid}, State) ->
    ?DBG("receive let_me_gossip from " ++ ?FROM_PIDS_TO_STRING([Pid])),
    To_send = lists:filter(fun(T) ->
        T#transaction.id >= Last_pending#transaction.id end, frozen_trans(State)),
    if
        length(To_send) > 0 ->
            ?DBG("gossip_you sent to " ++ ?FROM_PIDS_TO_STRING([Pid]) ++ ", sent: " ++
...
handle_cast({monitor_request, Pid, Trans}, State) ->
    ?DBG("received monitor_request from " ++ ?FROM_PIDS_TO_STRING([Pid])),
...
handle_cast({monitor_response, Outcome, Pid}, State) ->
    ?DBG("received monitor_response from " ++ ?FROM_PIDS_TO_STRING([Pid])),
    New_monitored =
        case Outcome of
            yes ->
            ...
            Num_waiting == 0 ->
                if
                    length(New_monitored) >= ?MONITORS_NUM ->
                        send_to_nodes({propose_trans, self(), Tran}, State#state.nodes),
...
handle_cast({propose_trans, Pid, Tran}, State) ->
    ?DBG("received propose_trans from " ++ ?FROM_PIDS_TO_STRING([Pid]) ++ " for tran: " ++
        ?FROM_TRANS_TO_STRING([Tran])),
    Tran_known = dict:find(Tran#transaction.id, State#state.tran_infos),
    ?DBG("propose_trans: " ++ ?FROM_PIDS_TO_STRING([Pid]) ++ ", " ++ ?FROM_TRANS_TO_STRING([Tran])),
        ...
        case Tran_known of
            {ok, Info} ->
                ...
                ?DBG("prop is old: " ++
                    lists:flatten(io_lib:format("~w",[tuple_to_list(
                        Info#tran_aux_info.max_prop_seq)])) ++ ", " ++ lists:flatten(
                        io_lib:format("~w",[tuple_to_list(Tran#transaction.seq_number)])) ++ ", "
                            ++ lists:flatten(io_lib:format("~w",[tuple_to_list(New_seq)]))),
                Empty_waiting = dict:is_empty(Waiting),
                New_askers =
                    if
                        Empty_waiting ->
                            send_to_nodes({trans_response, Tran#transaction.id, self(), New_seq},
                                Asker),
                            [];
                        true ->
                            Asker
            ...
            error ->
                ...
                ?DBG("prop is new: " ++ lists:flatten(io_lib:format("~w",[tuple_to_list(My_seq)]))
                    ++ ", " ++ lists:flatten(io_lib:format("~w",[tuple_to_list(
                        Tran#transaction.seq_number)])) ++ ", " ++ lists:flatten(io_lib:format(
                        "~w",[tuple_to_list(New_seq)]))),
                if
                    length(State#state.nodes) == 1 ->
                        gen_server:cast(Pid,
                            {trans_response, Tran#transaction.id, self(), New_seq});
                    true ->
                        send_to_nodes({propose_trans, self(),
...
handle_cast({trans_response, TransID, Pid, Seq}, State) ->
    ...
    ?DBG("trans_response: " ++ ?FROM_PIDS_TO_STRING([Pid]) ++ " for tran: " ++
    ...
    New_state =
        if
            ...
            Empty_waiting -> % must send back
                if
                    element(2, TransID) /= self() -> % not my transaction
                        send_to_nodes({trans_response, TransID, self(), New_seq}, Required),
                        State#state{
                        ...
                        if
                            length(State#state.monitored) == 1 ->
                                ...
                                gen_server:cast(hd(State#state.monitored),
                                    {trans_confirm, Confirmed_tran, self()}),
                                erlang:send_after(?SEND_EVERY, self(), send_time),
                            ...
                            true ->
                                send_to_nodes({trans_done, self()}, tl(State#state.monitored)),
...
handle_cast({trans_confirm, Tran, Pid}, State) ->
    ?DBG("received trans_confirm from " ++ ?FROM_PIDS_TO_STRING([Pid]) ++ " for trans: " ++
        ?FROM_TRANS_TO_STRING([Tran])),
...
handle_cast({trans_ack, Pid}, State) ->
    ?DBG("received trans_ack from " ++ ?FROM_PIDS_TO_STRING([Pid])),
    if
        length(State#state.monitored) == 2 -> % 2 because last monitor + who responds me now
            ...
            gen_server:cast(hd(lists:filter(fun(E) ->
                E /= Pid end, State#state.monitored)), {trans_confirm, Confirmed_tran, self()}),
            erlang:send_after(?SEND_EVERY, self(), send_time),
            {noreply, execute_ready(State#state{
...
handle_cast({trans_done, Pid}, State) ->
    ?DBG("received trans_done from " ++ ?FROM_PIDS_TO_STRING([Pid])),
...
handle_cast(_Request, State) ->
    ?DBG("received unknown message (my nodes: " ++ ?FROM_PIDS_TO_STRING(State#state.nodes) ++ ")"),
    {noreply, State}.
    ...
    if
        length(State#state.nodes) < ?MIN_NEIGHBORS ->
            ?DBG("timeout, it's searching neighbors time!(my nodes: " ++
                ?FROM_PIDS_TO_STRING(State#state.nodes) ++ ")"),
            send_to_nodes({nodes_request, self()}, State#state.nodes ++ [support_server]),
            erlang:send_after(?CHECK_NEIGHBOR_EVERY, self(), check_neighbors_time);
        true ->
            ok
    end,
        ...
        Size > 0 ->
            ...
            if
                length(Confirmed_trans) > 0 ->
                    ...
                    lists:foreach(fun(P) ->
                        gen_server:cast(P, {gossip_me, Last_tran, self()}),
                            ?DBG("gossip_me sent to " ++ pid_to_list(P)) end, State#state.nodes);
                true ->
                    First_pending = hd(trans_to_list(State#state.transactions)),
                    lists:foreach(fun(P) ->
                        gen_server:cast(P, {let_me_gossip, First_pending, self()}),
                            ?DBG("let_me_gossip sent to " ++ pid_to_list(P)) end,State#state.nodes)
            end;
        true ->
...
handle_info({'EXIT', Pid, Reason}, State) ->
    ?DBG(pid_to_list(Pid) ++ " interrupted link with me because: " ++ (if is_atom(Reason) ->
        atom_to_list(Reason); true -> "crash" end)),
    ...
    if
        (length(State#state.nodes) - 1) < ?MIN_NEIGHBORS ->
            ...
            send_to_nodes({nodes_request, self()}, lists:delete(Pid,State#state.nodes)),
            erlang:send_after(?CHECK_NEIGHBOR_EVERY, self(), check_neighbors_time);
        true ->
            ok
    end,
    ...
    Candidates =
        if
            length(State#state.nodes) >= ?MONITORS_NUM ->
                ...
                send_to_nodes({monitor_request, self(), Created_trans}, Not_monitoring),
...
%%% private functions
%%% ======================================================================================
-spec merge_neighbors(New_neighbor:: [pid()], Old_neighbor:: [pid()]) -> [pid()].
merge_neighbors([], Neighbors) -> Neighbors;
merge_neighbors([New_neighbor | Other], Neighbors) ->
    Neighbor_already_known = lists:member(New_neighbor, Neighbors) or (New_neighbor == self()),
    if
        ...
        true ->
            Is_alive = is_process_alive(New_neighbor),
            if
                Is_alive ->
                    ?DBG("passed node " ++ ?FROM_PIDS_TO_STRING([New_neighbor]) ++
                        " alive"),
                    link_and_notify(New_neighbor),
...
-spec link_and_notify(Pid:: pid()) -> atom().
link_and_notify(Pid) ->
    link(Pid),
    gen_server:cast(Pid, {link_require, self()}).


-spec build_proposed_seq(Dict:: orddict:orddict(trans_seq(), #transaction{})) -> trans_seq().
build_proposed_seq(Trans) ->
    case trans_to_list(Trans) of
        ...
        List ->
            ...
            {element(1,Last#transaction.seq_number) + 1, self()}
    end.
...
-spec trans_to_list(Dict:: orddict:orddict(trans_seq(), #transaction{})) -> [#transaction{}].
trans_to_list(Dict) ->
...
%% @doc if Send is a send where the availability condition is unspecified it converts it to a
%% guarded send that is always enabled
...
-spec update_trans(New_trans:: [#transaction{}], My_trans:: orddict:orddict(trans_seq(), #transaction{}), Infos:: dict:dict(trans_id(), #tran_aux_info{})) -> {orddict:orddict(trans_seq(), #transaction{}), dict:dict(trans_id(), #tran_aux_info{})}.
...
-spec send_to_nodes(Msg:: any(), Nodes:: [pid()]) -> ok.
...
-spec build_pid_dict(Pids:: list(pid())) -> dict:dict(pid(), atom()).
build_pid_dict(Pids) ->
    dict:from_list(lists:map(fun(P)->{P, null} end, Pids)).


-spec reap_crashed_responses(Trans:: dict:dict(trans_id(), #tran_aux_info{}), Pid:: pid()) -> ok.
reap_crashed_responses(Trans, Pid) ->
    dict:fold(fun(A,B,C)-> check_and_reap(A,B,C) end, Pid, Trans),
    ok.


-spec check_and_reap(TransID:: trans_id(), Tran:: #tran_aux_info{}, Pid:: pid()) -> pid().
